    ...
    <div class="flex items-center justify-center h-full w-full bg-gray-100 overflow-y-auto">
      <div class="flex flex-col items-center justify-center bg-white py-8 rounded-md px-12 shadow-lg w-1/4 min-w-[400px] min-h-[600px] h-1/2">
        ...
        <div class="space-y-8 mt-6 w-full">
          <div class="w-full">
            <input type="text" placeholder="Name" v-model="name" class="px-4 py-2 bg-gray-50 w-3/4">
          </div>
          <div class="w-full">
            <input type="password" placeholder="Password" v-model="password" class="px-4 py-2 bg-gray-50 w-3/4">
          </div>
          ...
          <div class="bg-gray-200 rounded-lg flex flex-row w-3/4 mx-auto">
              <div class="inline-flex rounded-lg w-3/4">
                <input type="radio" name="room_type" id="Private" value="private" hidden="" v-model="visibility">
              ...
              <div class="inline-flex rounded-lg w-3/4">
                <input type="radio" name="room_type" id="Public" value="public" checked="true" hidden="" v-model="visibility">
        ...
        <button :disabled="!name.length" class="w-3/4 mt-5 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-500 disabled:hover:bg-gray-200 bg-green-600 text-white hover:bg-green-700 hover:text-white py-2 rounded-md font-semibold tracking-tight transition-all duration-300">Create</button>
      </div>
    </div>
  </form>
</template>
...
export default defineComponent({
  ...
  methods: {
    createRoom() {
      console.log(this.name, this.password, this.visibility)
    }
  }
})
</script>

...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(
      () => route.params.login,
      async newLogin => {
        await fetch("http://10.12.1.4:9000/api/users/" + newLogin, {credentials: 'include'})
        .then((res) => res.json())
        .then((userData) => {
          data.user = userData;
        ...
        await(fetch("http://10.12.1.4:9000/api/game/score/"+ (data.user as any)?.id, {credentials: 'include'}))
        .then(res => res.json())
    ...
    onMounted(async () => {
      await fetch("http://10.12.1.4:9000/api/users/" + props.login, {credentials: 'include'})
        .then((res) => res.json())
        .then((userData) => {
          data.user = userData;
          if ((store.state.auth.user as any)?.friendList.includes((data.user as any)?.id))
  ...
  methods: {
    async toggleError() {
      ...
      axios
        .put("http://10.12.1.4:9000/api/users", curUsr, { withCredentials: true})

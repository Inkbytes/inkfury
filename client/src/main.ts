...
const sketch = function(p: any){
	const WIDTH = 1500;
	const HEIGHT = 800;
	var list: number[] = [-4, -3, 3, 4];
	var xpos: number = WIDTH / 2;
	var ypos: number = HEIGHT / 2;
	var dx:	number = list[Math.floor(Math.random() * 4)];
	var dy: number = list[Math.floor(Math.random() * 4)];
	var	p1pos: number = HEIGHT / 2;
	var p2pos: number = HEIGHT / 2;
	var p1score: number = 0;
	var p2score: number = 0;
	var	gameison: boolean = false;
	var game_status: number = 0;

	var paddle_width: number = 10;
	var paddle_height: number = WIDTH / 10;
	p.setup = () => {
